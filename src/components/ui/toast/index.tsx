...
const getVariantStyles = (
  ...
  if (appearance === "glow") {
    switch (variant) {
      case "success":
        return {
          border: "border-2 border-solid border-green-700",
          boxShadow:
            "0 0 10px rgba(34, 197, 94, 0.8), 0 0 20px rgba(34, 197, 94, 0.6), 0 0 30px rgba(34, 197, 94, 0.4)",
        };
      case "error":
        return {
          border: "border-2 border-solid border-red-700",
          boxShadow: "0 0 10px 5px rgba(255, 0, 0, 0.5)",
        };
      case "warning":
        return {
          border: "border-2 border-solid border-yellow-400",
          boxShadow:
            "0 0 10px rgba(252, 211, 77, 0.8), 0 0 20px rgba(252, 211, 77, 0.6), 0 0 30px rgba(252, 211, 77, 0.4)",
        };
      case "info":
        return {
          border: "border-2 border-solid border-blue-700",
          boxShadow:
    ...
    return {
      borderLeft: `6px solid ${gradientColor}`,
...
const Toast = ({
  ...
  const modeColor =
    appearance === "glow"
      ? mode === "dark"
        ? "bg-gradient-to-t from-gray-800 via-slate-800 to-gray-800 text-white"
        : "bg-white text-black"
      : "";
  gradientColor =
    gradientColor.indexOf("rgb") === -1 ? "rgba(5, 1, 1, 1)" : gradientColor;
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => onClose(id)}
        className={`ml-4 mb-2 text-xs transform hover:scale-75 ${mode === "dark" ? "text-white" : "text-black"} font-semibold hover:text-gray-400`}
      >
        <CrossCircledIcon />
      </button>
    </motion.div>
